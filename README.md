...
GitHub MCP runtime mapping uses the inspected tools documented in `docs/reference/github-mcp-tools.md`. The default `CodeHostPort` mapping checks branch readiness with `list_branches`, creates remote branches only through `create_branch` when explicitly needed, opens draft PRs with `create_pull_request`, resolves PR numbers with `list_pull_requests`, reads checks through `pull_request_read` using `method: get_check_runs`, and comments on PRs through `add_issue_comment` with the PR number as `issue_number`. Custom `github.mcp_tools` overrides remain supported for these typed actions. GitHub branch push remains on the local/native git fallback path, and Ewokbot does not call GitHub merge, branch deletion, remote file mutation, repository mutation, workflow mutation, secret management, or production automation tools by default; `merge_pull_request` is always human-only.
