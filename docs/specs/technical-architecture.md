...
Machine-wide Ewokbot state is separate from workspace-local delivery evidence. User-level paths resolve from `XDG_CONFIG_HOME`, `XDG_DATA_HOME`, and `XDG_CACHE_HOME` when those environment variables are set, otherwise from `~/.config`, `~/.local/share`, and `~/.cache`. The target layout is `~/.config/ewokbot/config.json`, `~/.local/share/ewokbot/auth.json`, `~/.local/share/ewokbot/state/`, and `~/.cache/ewokbot/`. These paths are created only by explicit setup/auth flows such as `ewokbot init`; runtime run state, reports, worker locks, and control records remain under `.ewokbot/runs/` in the workspace. The user `auth.json` is Ewokbot auth metadata only, is created with owner-only permissions where supported, and must not contain OpenCode credentials.
