...
Default workspace configuration lives at `.ewokbot/workspace.yml`; `ewokbot init` generates it under the workspace-local `.ewokbot/` directory alongside `.ewokbot/.env`, placeholder-only `.ewokbot/.env.example`, and the owned `runs/`, `logs/`, and `cache/` directories. The init command refuses to overwrite any existing onboarding file by default before collecting interactive values so credentials and local setup are not replaced accidentally.

Machine-wide Ewokbot state is separate from workspace-local delivery evidence. User-level paths resolve from `XDG_CONFIG_HOME`, `XDG_DATA_HOME`, and `XDG_CACHE_HOME` when those environment variables are set, otherwise from `~/.config`, `~/.local/share`, and `~/.cache`. The target layout is `~/.config/ewokbot/config.json`, `~/.local/share/ewokbot/auth.json`, `~/.local/share/ewokbot/state/`, and `~/.cache/ewokbot/`. These paths are created only by explicit setup/auth flows such as `ewokbot init` or `ewokbot auth login <provider>`; runtime run state, reports, worker locks, and control records remain under `.ewokbot/runs/` in the workspace. The user `auth.json` is Ewokbot auth metadata only, is created with owner-only permissions where supported, and must not contain OpenCode credentials. `ewokbot auth status`, `auth list`, `auth login <provider>`, and `auth logout <provider>` manage metadata-only Jira, GitHub, Railway, and Vercel records in this file without live OAuth/provider/MCP/network calls; OpenCode auth commands are refused with guidance to use OpenCode directly.
...
Dev tool setup readiness is modeled through `DevToolSetupAdapter`. The OpenCode adapter normalizes readiness into `not_installed`, `installed_not_authenticated`, `installed_authenticated_no_model`, `installed_ready`, `installed_unsupported`, and `command_failed`. It detects custom command paths, command/version readiness, global OpenCode config at `~/.config/opencode/opencode.json`, OpenCode auth presence at `~/.local/share/opencode/auth.json` and optional read-only `opencode auth list` probes, and project config at `<workspace-root>/opencode.json`. Adapter results expose booleans, paths, statuses, and next steps only; they must not read, print, copy, or store raw OpenCode secrets, and setup actions are returned for explicit operator confirmation rather than invoked automatically. Interactive init surfaces these readiness states through an `@inquirer/prompts` TUI: ready OpenCode can be selected without model provider API-key prompts, not-authenticated or no-model states require explicit acknowledgement or instructions, and missing/failed/unsupported states offer mock mode, setup instructions, or a custom command check without running installers or auth flows.
