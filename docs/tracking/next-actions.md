...
1. Milestone Z from `docs/plans/approved-backlog.md` is the immediate next approved work: Railway Staging Verification.
2. Do not implement work beyond Milestone Z until Milestone Z is complete and tracking is updated.
...
Next approved work: Milestone Z, Railway Staging Verification, from `docs/plans/approved-backlog.md`.

Milestone Z must keep Railway MCP-first for deployment status and service URL lookup, use mock DeploymentPort and smoke verifier tests only, avoid live Railway or MCP calls in tests, avoid credentials and production deployment automation, preserve human-only production controls, and avoid implementing unapproved provider behavior beyond staging verification.
